...
@bp.route('/pic/<string:img_type>/<string:bot_name>', methods=['GET'])
@inject
def get_cache_image(img_type: str, bot_name: str):
    filename = 'position.jpeg' if img_type == 'raw' else 'position.jpeg_' + img_type + '.jpg'
    pathname = os.path.join('/app/build/bots', 'bot_' + bot_name, filename) # @todo: absolute path, move to config
    print(pathname)
    print(pathname)
