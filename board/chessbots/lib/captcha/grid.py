...
from chessbots.lib.captcha.marker import Marker
...
def resolve_markers_to_grid(markers: [Marker], half_grid_size: int, tolerance: int) -> [str, float, [GridPoint], [Point, Point, Point]]:
    t_point = Point(tolerance, tolerance)
    if not 0 < len(markers):
        return -1, [], [], ''
    ...
    grid_y_min = half_grid_size * -1
    grid_y_max = half_grid_size
    grid_x_min = half_grid_size * -1
    grid_x_max = half_grid_size
...
def create_grid_point(target: Point, position: Point, markers: [Marker], t_point: Point) -> GridPoint:
    ...
    return GridPoint(target, position, Marker(position, 0, '2'))  # here we set the value for "not found"
